...
const convertDataProviderRequestToHTTP = (type, resource, params) => {
    ...
    case GET_LIST: {
        ...
        console.log(filter)
        if(filter.length===0)
        filter =humps.decamelize(filter);

        if(params.filter.hasOwnProperty('time_start')&&params.filter.hasOwnProperty('time_end')){
            filter['createTime']=JSON.stringify([`${params.filter.time_start}`,`${params.filter.time_end}`]);
    ...
    case CREATE:
        ...
        if(params.data.hasOwnProperty('goodsFullNum')){
            ...
            return {
                ...
                options: { method: 'POST' , headers : new Headers({
                //    'Content-Type': 'multipart/form-data;boundary=ASD;charset=UTF-8',
                }),body:formData},
            };
        }
...
console.log(json)
    ...
    case GET_MANY_REFERENCE:
        return {data:json.resultBody.records}
    default:
